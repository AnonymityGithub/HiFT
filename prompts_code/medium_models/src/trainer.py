...
import math
import time
from transformers.utils import is_sagemaker_mp_enabled,logging,is_accelerate_available,logging
import transformers
...
class Trainer(LinearHeadTrainer):
    ...
    def init_group_parameters(self):
        ...
        if self.model.label_word_list is not None:
            self.group_parameters.remove(self.spLayers[-1])
    ...
    def training_step(self, model: nn.Module, inputs: Dict[str, Union[torch.Tensor, Any]]) -> torch.Tensor:
        ...
        return super(Trainer, self).training_step(model,inputs)
