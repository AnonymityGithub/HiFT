...
from optimizers.replace_operation import replace_backward
replace_backward()

SpecialLayers={
    "opt":[r"embed_[^ ]+","final_layer_norm"],
    "llama":["embed_tokens","norm","lm_head"]
}
...
#     eval_dict = None
#     with open("eval_dict.json","r",encoding="utf-8") as f:
#         eval_dict = json.load(f)
#     encoded_candidates = eval_dict["encoded_candidates"]
#     option_lens = eval_dict["option_lens"]
#     correct_candidate_ids = eval_dict["correct_candidate_ids"]
#     metric_name = eval_dict["metric_name"]
#     scores = []
#     for logits,input_ids,option_len in zip(preds,encoded_candidates,option_lens):
#         labels = input_ids[1:]
#         logits = logits[:-1]
#         log_probs = F.log_softmax(torch.tensor(logits,dtype=torch.float32), dim=-1)

#         selected_log_probs = log_probs[torch.arange(len(labels)), labels]
#         selected_log_probs = np.array(selected_log_probs.cpu().detach())
#             # Only return the option (candidate) part
#         selected_log_probs = selected_log_probs[-option_len:]
#         scores.append(selected_log_probs.mean().item())
    
#     predicted_candidate=int(np.argmax(scores))
#     Prediction(correct_candidate=correct_candidate_id, predicted_candidate=predicted_candidate)
...
class OurArguments(TrainingArguments):
    ...
    #hier
    group_element:int = 1
    optimizer_strategy: str = "down2up"
    hier_tuning: bool = True
    hier_reverse: bool = False
...
class Framework:
    ...
    def __init__(self, args, task):
        ...
        if "opt" in self.args.model_name:
            self.spLayers = SpecialLayers["opt"]
        elif "llama" in self.args.model_name:
            self.spLayers = SpecialLayers["llama"]
    ...
    def gene_eval_info(self, train_samples, eval_sample, verbose=False):
    ...
    def train(self, train_samples, eval_samples):
        ...
        trainer = OurTrainer(
            spLayers = self.spLayers,
            group_element = self.args.group_element,
            optimizer_strategy = self.args.optimizer_strategy,
            hier_reverse = self.args.hier_reverse,
            hier_tuning=self.args.hier_tuning,
            model=self.model, 
            args=self.args,
    ...
    if args.train_set_seed is not None or args.num_train_sets is not None:
        ...
        for train_set_id, train_samples in enumerate(train_sets):
            ...
            if args.trainer != "none":
                ...
                # encoded_candidates,option_lens,correct_candidate_ids = [],[],[]
                # for eval_id, eval_sample in enumerate(tqdm(dev_samples)):
                #     encoded_candidate,option_len=framework.gene_eval_info([], eval_sample, verbose=(eval_id < 3))
                #     encoded_candidates.extend(encoded_candidate)
                ...
                #     else:
                #         correct_candidate_id = eval_sample.candidates.index(eval_sample.correct_candidate)
                #     correct_candidate_ids.append(correct_candidate_id)

                # eval_dict = {"encoded_candidates":encoded_candidates,
                #             "option_lens":option_lens,
                #             "correct_candidate_ids":correct_candidate_ids,
                #             "metric_name":metric_name}
                # with open("eval_dict.json","w",encoding="utf-8") as f:
